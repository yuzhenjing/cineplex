...
<section class="container">
    ...
    if ($_GET["sousuo"]) {
        ...
        curl_close($ch);
        if ($x = strpos($response, "<ul class=\"media-list media-list-set\">")) {
            $response = substr($response, $x);
        }
        if ($x = strpos($response, "<div class=\"push\">")) {
        ...
        if ($_GET["main"]) {
            ...
            $response = curl_exec($ch);
            curl_close($ch);
            if ($x = strpos($response, "<div class=\"row-flbtd tor-title\">")) {
                $response = substr($response, $x);
            }
            if ($x = strpos($response, "<div class=\"row-flbtd tor-ad\">")) {
                $response = substr($response, 0, $x);
            }
            ...
            <div id="homeso">
                <form method="get" id="homeso" style="text-align: center;float: none" action="">
                    ...
                    <button id="button" tabindex="3" class="homesobtn" type="submit">搜磁力</button>
                </form>
                <script>
